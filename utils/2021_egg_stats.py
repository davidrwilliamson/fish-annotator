...
def prepare_dataframe(df: pd.DataFrame, root_folder, dates, treatments) -> pd.DataFrame:
    for d in dates:
        for t in treatments:
            dir_path = os.path.join(root_folder, d, t)
            if os.path.isdir(dir_path):
...
def additional_calcs(df: pd.DataFrame) -> pd.DataFrame:
    ...
    df['Egg mean diameter[mm]'] = (df['Egg max diameter[mm]'] + df['Egg min diameter[mm]']) / 2.0
    return df
...
def manual_exclusions(df: pd.DataFrame):
    ...
    idx = df.loc[df['Eye area[mm2]'] < 0.013].index
    df.loc[idx, 'Eye area[mm2]'] = np.nan
    df.loc[idx, 'Eye min diameter[mm]'] = np.nan
    df.loc[idx, 'Eye max diameter[mm]'] = np.nan

    idx = df.loc[df['Embryo area[mm2]'] > 0.81].index
    df.loc[idx, 'Embryo area[mm2]'] = np.nan
    ...
    if os.path.isfile(note_file):
        ...
        for note in notes:
            ...
            if note[4].astype(bool):  # recheck
                ...
                pass
            if note[5].astype(bool):  # egg
                df.loc[idx, 'Egg area[mm2]'] = np.nan
                df.loc[idx, 'Egg min diameter[mm]'] = np.nan
                df.loc[idx, 'Egg max diameter[mm]'] = np.nan
                df.loc[idx, 'Yolk fraction'] = np.nan

        recheck_file = os.path.join(folder_path, 'rechecked_measurements.csv')
        if os.path.isfile(recheck_file):
            rc_df = pd.read_csv(recheck_file, dtype={'Image ID': str, 'Fish ID': int, 'Embryo area[mm]': float,
                                                     'Eye min diameter[mm]': float, 'Egg min diameter[mm]': float,
                                                     'Embryo bad': bool, 'Eye bad': bool, 'Yolk bad': bool,
                                                     'Egg bad': bool})
            bad_embryos = rc_df.loc[rc_df[rc_df['Embryo bad'] == True].index]
            for i in bad_embryos.index:
                rows = df[(df['Image ID'] == bad_embryos['Image ID'][i]) &
                          (df['Fish ID'] == bad_embryos['Fish ID'][i]) &
                          (df['Embryo area[mm2]'] == bad_embryos['Embryo area[mm2]'][i])]
                df.loc[rows.index, 'Embryo area[mm2]'] = np.nan
            ...
            bad_eggs = rc_df.loc[rc_df[rc_df['Egg bad'] == True].index]
            for i in bad_eggs.index:
                rows = df[(df['Image ID'] == bad_eggs['Image ID'][i]) &
                          (df['Fish ID'] == bad_eggs['Fish ID'][i]) &
                          (df['Egg min diameter[mm]'] == bad_eggs['Egg min diameter[mm]'][i])]
                df.loc[rows.index, 'Egg area[mm2]'] = np.nan
                df.loc[rows.index, 'Egg min diameter[mm]'] = np.nan
                df.loc[rows.index, 'Egg max diameter[mm]'] = np.nan
                df.loc[rows.index, 'Yolk fraction'] = np.nan
...
def boxplot_by_treatment_all_dates(df: pd.DataFrame, attribute: str):
    ...
    pairs = [('Control', '8 μg/L'), ('Control', '27 μg/L'), ('Control', '108 μg/L'), ('Control', '220 μg/L'),
             ('Control', '343 μg/L'), ('Control', '747 μg/L')]
